...
router.post("/", [body("url").isURL().withMessage("Enter a valid url")], async (req, res, next) => {
    try {
...
router.put("/:code", [body("url").isURL().withMessage("Enter a valid url")], async (req, res, next) => {
    const code = req.params.code;
    const url = req.body.url;
    try {
        const link = await Link.findOne({ shortCode: code });
